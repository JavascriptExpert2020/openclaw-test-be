...
const ghlRequest = async (init) => {
    ...
    if (!res.ok) {
        ...
        throw new GhlHttpError(res.status, `GHL request failed (${res.status} ${res.statusText}): ${detail}`);
    }
    const payload = await readResponsePayload(res);
