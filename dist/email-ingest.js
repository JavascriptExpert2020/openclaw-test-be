...
import { fetchLatestAssistantMessage, sendChatMessage, waitForRun, } from "./openclaw-gateway.js";
...
const collectEmails = (...values) => {
    const emails = [];
    const addFromAddressObject = (value) => {
...
const parseCsv = (value) => (value || "")
    ...
    .filter(Boolean);
const matchAllowlist = (from, allowEmails, allowDomains) => {
...
const ensureReplySubject = (subject) => {
    if (!subject) {
...
const buildSessionKey = (value) => {
    const hash = createHash("sha256").update(value).digest("hex").slice(0, 16);
    return `agent:main:email:${hash}`;
};
const buildAgentPrompt = (mail, forwarded) => {
...
const getMessageSource = (value) => {
    if (!value || typeof value !== "object") {
        return undefined;
    }
    return value.source;
};
export const startEmailIngest = () => {
    ...
    log(`[email] ingest enabled (imap=${imapHost}:${imapPort} user=${imapUser} mailbox=${mailbox} poll=${pollSeconds}s max=${maxMessages} since=${sinceHours}h inbox=${inboxAddress} requireForwarded=${requireForwarded} skipBulk=${skipBulk} sendReplies=${sendReplies})`);
    let stopped = false;
    let polling = false;
    const pollOnce = async () => {
        ...
        try {
            ...
            for (const uid of recent) {
                ...
                const isForwarded = Boolean(forwarded) || /^(fwd?|fw):/i.test(subject) || forwardedMarkers.test(bodyText);
                if (!addressedToInbox) {
                ...
                log(`[email] processing uid=${uid} from=${getAddress(parsed.from)} subject=${subject}`);
