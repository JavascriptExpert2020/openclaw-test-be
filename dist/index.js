...
import { fetchChatLogs, fetchSessions, fetchSkills, fetchUsageItems, updateSkill, } from "./openclaw-gateway.js";
...
app.post("/api/bookkeeping/append", async (req, res) => {
    try {
        ...
        const { date, vendor, amount, category, notes, source } = req.body;
...
app.post("/api/ghl/contacts/search", async (req, res) => {
    try {
        const { query, email, phone, limit } = req.body;
...
app.post("/api/ghl/contacts/update", async (req, res) => {
    try {
        const { contactId, query, email, phone, updates } = req.body;
        ...
        const item = await updateContact(resolvedId, updates);
        return res.json({ item });
    }
    catch (err) {
        return res.status(500).json({
